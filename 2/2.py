...
# ФУНКЦИЯ ДЛЯ WCSS
def compute_wcss(points, labels, centers):
    #Вычисляет WCSS (сумму квадратов расстояний до центроидов)
    points = np.array(points)
    wcss = 0
    for i, point in enumerate(points):
        center = centers[labels[i]]
        wcss += np.linalg.norm(point - center) ** 2
    return wcss
...
def update(event):
    ...
    wcss1 = compute_wcss(points, labels1, centers1)

    # Метод 2
    start = time.perf_counter()
    labels2, centers2, it2 = my_kmeans(points.tolist(), k)
    time2 = time.perf_counter() - start
    wcss2 = compute_wcss(points, labels2, centers2)

    # Метод 3
    start = time.perf_counter()
    labels3, centers3, it3 = sklearn_kmeans(points.tolist(), k)
    time3 = time.perf_counter() - start
    wcss3 = compute_wcss(points, labels3, centers3)

    #  ВЫВОД В КОНСОЛЬ
    print(f" {title}  K = {k}")
    print(f"{'Метод':<20} {'Время (с)':<12} {'WCSS':<15} {'Итерации':<10}")
    print(f"{'Мой метод':<20} {time1:<12.6f} {wcss1:<15.2f} {'—':<10}")
    print(f"{'Мой K-means':<20} {time2:<12.6f} {wcss2:<15.2f} {it2:<10}")
    print(f"{'Sklearn K-means':<20} {time3:<12.6f} {wcss3:<15.2f} {it3:<10}")
...
#  МЕТОД ЛОКТЯ
def show_elbow_method():
    print("МЕТОД ЛОКТЯ для Теста 10")

    points = np.array(test_data['Тест10'])
    ...
    elbow_k = np.argmax(diffs) + 1 if diffs else 3

    plt.figure(figsize=(10, 6))
    plt.plot(K_range, wcss, 'bo-', linewidth=2, markersize=8)
    plt.axvline(x=elbow_k, color='red', linestyle='--', linewidth=2, label=f'Локоть (K={elbow_k})')
